...
def _idle_step_off_station(state, unit):
    ...
    if storages and unit.current_node not in storages:
        # spread idle units across storages: prefer the least-crowded one
        def crowd(s):
            c = 0
            for u in state.drive_units:
                if u.id == unit.id:
                    continue
                pos = u.transit_destination if u.in_transit else u.current_node
                if pos == s:
                    c += 1
            return c
        tgt = min(storages, key=lambda s: (crowd(s), _dd(state, unit.current_node, s)))
