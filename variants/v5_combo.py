...
def _idle_step_off_station(state, unit):
    """Idle: park ADJACENT to nearest storage (on it = compulsory auto-grab), else clear stations."""
    storages = [n.id for n in state.nodes if n.node_type == "storage"]
    if storages:
        # candidate parking spots: non-storage, non-station neighbors of storage nodes
        spots = set()
        for s in storages:
            for nbr, _c in _cache["adj"].get(s, ()):
                nb = state.get_node(nbr)
                if nb is not None and nb.node_type == "travel":
                    spots.add(nbr)
        if unit.current_node in spots:
            return None  # already parked
        if spots:
            tgt = min(spots, key=lambda s: _dd(state, unit.current_node, s))
            if _dd(state, unit.current_node, tgt) != float("inf"):
                ...
                if hop is not None and _move_ok(state, unit, hop):
                    ...
                    if nb is None or nb.node_type == "travel" or hop == tgt:
                        return hop
